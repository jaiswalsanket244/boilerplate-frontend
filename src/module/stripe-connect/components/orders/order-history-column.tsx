...
import RefundDialog from "@/module/stripe-connect/components/orders/refund-dialog";
		...
		},
		{
			...
			header: () => (
				<div className="text-center">
					Days left
					<div className="text-xs text-gray-500">(For cancellation)</div>
				</div>
			),
			size: 300,
			cell: ({ row }) => {
				const { daysSinceOrder, isRefundEligible, refunded } = row.original;
				const maxRefundDays = 7; // Assuming 7 days refund policy
				const daysLeft = maxRefundDays - daysSinceOrder;

				if (refunded) {
					return <div className="text-center text-gray-400">-</div>;
				}

				if (!isRefundEligible || daysLeft <= 0) {
					return <div className="text-center text-gray-400">-</div>;
				}
					...
					<div className="text-center">
						<span
							className={`font-medium ${
								daysLeft <= 2 ? "text-red-600" : daysLeft <= 5 ? "text-yellow-600" : "text-green-600"
							}`}
						>
							{daysLeft}
						</span>
