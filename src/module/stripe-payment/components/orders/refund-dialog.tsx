...
const RefundDialog = ({ row }: { row: Row<IOrderDetails> }) => {
	const { useRefundMutation } = useStripePaymentApi();
	const queryClient = useQueryClient();

	const { mutateAsync: refund, isPending } = useRefundMutation();
	...
	const handleRefund = async (id: string) => {
		try {
			...
			await queryClient.invalidateQueries({
				predicate: (query) => query.queryKey.includes("stripe-payment-user-orders"),
			});
	...
	if (refunded) {
		return <div className="text-center text-txt-secondary-700">-</div>;
	}
